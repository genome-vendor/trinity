...
my $usage = "usage: $0 blat_output.pslx [MAX_TIERS=1] [MIN_IDENTITY=95] [STRAND_SPECIFIC=0]\n\n";

my $blat_output = $ARGV[0] or die $usage;
my $MAX_TIERS = $ARGV[1] || 1;
my $min_identity = $ARGV[2] || 95;
my $strand_specific = $ARGV[3] || 0;

# ~/bio_tools/blat/current/linux/blat -t=dna -q=dna -minIdentity=95 -fastMap -out=blast8 SP2_Sanger_072008_1.cds Spom_ds_61G9EAAXX_and_61G9UAAXX.reads.fasta.mc2_irke outputfile
	...
	foreach my $gene (keys %gene_to_overlapping_assemblies) {
		...
		while (@matches) {
			...
			unless ($placed_flag) {
				...
				if (scalar(@tiers) < $MAX_TIERS || $MAX_TIERS < 0) {
					print "** adding match to new tier.\n" if $DEBUG;
					push (@tiers, [$match]);
				}
				else {
					print "** cannot add match to tiers.\n" if $DEBUG;
				}
			}
		}
		...
		foreach my $tier_aref (@tiers) {
			...
			foreach my $match (@tier) {
				...
				print "\t$acc\{S[$score],G:$gene_lend-$gene_rend:$gene_len,T:$trans_end5-$trans_end3:$trans_len,$per_id,M:$matches,N:$mismatches,gaps:$total_gaps}";
			}
			print "\n";
			
			
			if ($MAX_TIERS > 0 && $tier_counter >= $MAX_TIERS) { 
				last;
			}
...
sub examine_blat_mappings {
	...
	while (my $psl_entry = $psl_reader->get_next()) {
		...
		if ($percent_gaps >= (100-$min_identity)) { next; }
