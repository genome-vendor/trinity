...
unless ($matrix_file && $transcripts) { 
    die $usage;
}
...
main: {
    ...
    my %trans_lengths = &get_transcript_lengths($transcripts);
    ...
    if ($reference) {
        ...
        foreach my $comparator (@comparators) {
            &run_edgeR($reference, $comparator, \%filenames);
        }
    }
    else {
        ## running all-vs-all
        ...
        for (my $i = 0; $i < $#cols; $i++) {
            ...
            for (my $j = $i + 1; $j <= $#cols; $j++) {
                ...
                &run_edgeR($cols[$i], $cols[$j], \%filenames);
            }
        }
    }
...
sub write_normalized_fpkm_file {
    ...
    my $normalized_fpkm_file = basename($matrix_file) . ".normalized.FPKM";
    ...
    while (<$fh>) {
        ...
        for (my $i = 1; $i <= $#x; $i++) {
            ...
            my $fpkm = $read_count / ($seq_len / 1e3) / ($eff_lib_size / 1e6);
...
sub get_transcript_lengths {
    ...
    if (-s "$file.seqLens") {
        open (my $fh, "$file.seqLens") or die $!;
        while (<$fh>) {
            chomp;
            my ($acc, $seqlen) = split(/\t/);
            $trans_lengths{$acc} = $seqlen;
        }
        close $fh;
        ...
        # compute lengths based on the fasta entries (default)
