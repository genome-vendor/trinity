...
main: {
    ...
    print $ofh "data = read.table(\"$counts_matrix_file\", header=T)\n";
    ...
    print $ofh "conditions = c(rep(\"$repA_name\", $num_rep_A), rep(\"$repB_name\", $num_rep_B))\n";
    print $ofh "\n";
    ...
    print $ofh "\n";
    ## output results
    print $ofh "write.table(res[order(res\$padj),], file=\'$output_prefix.DESeq.output\', sep='\t', quote=FALSE, row.names=FALSE)\n";
