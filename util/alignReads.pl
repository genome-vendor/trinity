...
#
#  --aligner <string>           BLAT, bowtie, bwa, bwamod, or tophat  
#
# Optional:
#
...
#
#  If paired (bowtie, BLAT, and bwamod) modes, NOT tophat:
#
...
unless ($aligner && $aligner =~ /^BLAT|bowtie|bwa|bwamod|tophat$/) {
...
my $bwamod_prog = "$FindBin::Bin/../trinity-plugins/bwa-0.5.7-patched_multi_map/bwa";
...
main: {
    ...
    unless (-s "target.fa") {
        ...
        elsif ($aligner eq "bwamod") {
            my $cmd = "$bwamod_prog index target.fa";
            &process_cmd($cmd);
        }
    ...
    if ($aligner eq "tophat") {
    ...
    elsif ($aligner =~ /^bowtie|bwamod|BLAT/) {
        ...
        foreach my $fa_info (@entries) {
            ...
            eval {
                ...
                if ($aligner eq "BLAT") {
                    ...
                    ## Run BWA-mod pipeline
                    ## this is a customized version of BWA that allows for multiple read mapping in the single-read sam files
                
                    &run_bwamod_alignment_pipeline($target_fa, $trans_file);
        ...
        push (@to_delete, "$outfile_basename.coordSorted.sam") unless ($retain_SAM_files_flag);
        ...
        push (@to_delete, "$outfile_basename.nameSorted.sam") unless ($retain_SAM_files_flag);
        ...
        if ($SS_lib_type) {
            ...
            if (! $single_file) {
                ...
                push (@to_delete, "$outfile_basename.nameSorted.PropMapPairsForRSEM.sam") unless ($retain_SAM_files_flag);;
...
sub make_RSEM_bam {
    ...
    my $cmd = "$RSEM_dir/convert-sam-for-rsem $pre_bam_file $out_bam_prefix";
    &process_cmd($cmd);
...
####
sub run_bwamod_alignment_pipeline {
    my ($target_fa, $trans_file) = @_;

    
    my $cmd = "$bwamod_prog aln @ARGV target.fa $target_fa > $target_fa.sai";
    &process_cmd($cmd) unless (-e "$target_fa.sai");
    
    
    
    $cmd = "$bwamod_prog samse -n $num_hits -s target.fa $target_fa.sai $target_fa > $target_fa.pre.sam";
