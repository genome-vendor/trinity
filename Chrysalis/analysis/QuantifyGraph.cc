...
int main(int argc,char** argv)
{
    ...
    string start_file = oString + ".start";
    string touch_start_file = "touch " + start_file;
    Execute(touch_start_file.c_str());
    string finished_file = oString + ".finished"; // start file gets moved to .finshed after successful completion.

    if (Exists(finished_file)) {
        // cerr << "Quanitfy graph previously finished successfully on " << aString << ".  Not rerunning here." << endl;
        //return(0);
    }
    ...
    seq.Read(aString, false, true); // parse the reads from the fasta file
    ...
    if (NO_CLEANUP) {
        ...
        // change 'start' file flag to 'finished'
        string start_to_finished = "mv " + start_file + " " + finished_file;
        Execute(start_to_finished.c_str());
        // cout << start_to_finished << endl;
    }
    else {
        ...
        // cleanup
        string rm_input_reads = "rm " + aString;
        Execute(rm_input_reads.c_str());
        
        string rm_input_graph = "rm " + gString;
        Execute(rm_input_graph.c_str());
        
        string rm_start_file = "rm " + start_file;
        Execute(rm_start_file.c_str());
