...
void Add(vecDNAVector & all, DNAVector & add, int & counter) 
{
    #pragma omp critical
    {
        if (counter >= all.isize()) {
            ...
            // cerr << "Resizing to : " << all.isize() << endl;
        }
        all[counter] = add;
        counter++;
    }
...
int main(int argc,char** argv)
{
    ...
    if (! bNoWeld) { // make this a function
        ...
        #pragma omp parallel for schedule(dynamic, 1000) private(j)
    ...
    for (i=0; i<clustered_pools.isize(); i++) {
        ...
        for (unsigned int j = 0; j < p.size(); j++) {
            ...
            pool_info << z << " ";
            cout << ">Component_" << i << " " << p.size() << " " << z << " [iworm" << dna.Name(z) << "]" << endl;
            PrintSeq(dna[z]);
        }
        pool_info << endl;
        cout << pool_info.str();
